...
```bash
todo-api$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
todo-api$ sam local start-api
todo-api$ curl http://localhost:3000/
```
...
```bash
todo-api$ sam logs -n HelloWorldFunction --stack-name todo-api --tail
```
...
```bash
todo-api$ pip install pytest pytest-mock --user
todo-api$ python -m pytest tests/ -v
```
